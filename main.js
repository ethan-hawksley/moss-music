...
const parseM3UFile = (filePath) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      let playlistData = {
        id: path.basename(filePath), title: path.basename(filePath, ".m3u"), songs: [],
      };
        ...
        if (line.startsWith("#EXTINF:")) {
          ...
          if (currentSong) {
            ...
            playlistData.songs.push({
              id: path.basename(songPath), title: path.basename(songPath), path: songPath, channel: "file",
            });
          }
          currentSong = null;
        }
...
const swapPlaylists = (playlistId1, playlistId2) => new Promise(async (resolve) => {
  ...
  const updatePlaylist1Position = new Promise((resolve, reject) => {
    db.run(`UPDATE playlists
            SET position = ?
            WHERE id = ?`, [playlist2Position, playlistId1], (err) => {
  ...
  const updatePlaylist2Position = new Promise((resolve, reject) => {
    db.run(`UPDATE playlists
            SET position = ?
            WHERE id = ?`, [playlist1Position, playlistId2], (err) => {
      if (err) return reject(err);
      resolve();
    },);
  });
...
const fetchPlaylistData = (playlistURL) => new Promise((resolve, reject) => {
  ...
  command.on("close", (code) => {
    ...
    try {
      ...
      let playlistData = {
        id: jsonLine.id, title: jsonLine.title.replace(/^Album - /, ""), songs: jsonLine.entries.map((rawSong) => ({
          id: rawSong.id, title: rawSong.title
            ...
            .replace(/^.*?(?:OST|Soundtrack|Remaster|Acoustic)\s*(?:\([^)]*\))?\s*-\s*/i, "",) // Remove OST/Soundtrack prefixes with optional parentheticals
        ...
        descriptionJson.forEach((song) => {
          if (song && song.position && song.path && song.title) {
            playlistData.songs.splice(song.position - 1, 0, {
              id: song.path, title: song.title, path: path.join(SONG_FILE_DIRECTORY, song.path), channel: "file",
            });
          }
        });
      }
      ...
      fs.writeFileSync(path.join(app.getPath("documents"), "yt-dlp-error.log"), output,);
...
const downloadPlaylist = (playlistURL) => new Promise(async (resolve, reject) => {
  try {
    ...
    const isLocalFile = playlistURL.toLowerCase().endsWith(".m3u") && (fs.existsSync(playlistURL) || fs.existsSync(path.resolve(playlistURL)) || playlistURL.startsWith("/") || playlistURL.startsWith("./"));
          ...
          return new Promise((resolve, reject) => {
            db.get("SELECT id FROM songs WHERE id = ?", [song.id], async (err, row) => {
              ...
              try {
                ...
                console.error(`Error downloading song ${song.id}:`, downloadError,);
